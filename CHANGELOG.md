...
- **Issue #73:** a personal setting now falls back to the admin-configured instance value (`occ config:app:set eva_ai …`) when the user has no explicit personal value; explicit choices still win. Per-user runtime state never inherits instance values, the settings API reports which keys are personal overrides, and the Settings page offers a per-key "Reset to instance default" control.
