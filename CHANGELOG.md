...
- Recover a future-TTL lock when the user has an idle, explicitly cancelled run, preventing a stale worker lock from blocking indexing until its TTL expires.

## [1.16.77] - 2026-09-14

### Tool visibility

- Open terminal tool arguments and bounded results automatically in the live chat trace so confirmed commands are easier to audit.
...
- Added concrete, security-focused Immich person-search and Vaultwarden
  metadata-plugin recipes, including ACL, secret-handling and test guidance.
...
- Direct file search now preserves matches from accessible folders when a
  nested remote or permission-restricted folder cannot be listed, reporting a
  bounded partial result instead of failing the whole search.
...
- Connector discovery now resolves safe OpenAPI `servers.url` path variables
  from declared defaults, while rejecting unresolved or unsafe values.
- Added regression coverage for templated API prefixes.
...
- Terminal command sequences can now provide one bounded prompt input per
  command, with the same EOF, timeout, allowlist and confirmation safeguards.
...
- Documented direct unindexed search bounds, cache refresh behavior and coverage diagnostics.
...
- Third-party EVA plugins can now explicitly support all five execution surfaces, including RAG and task-processing proposal runs; unsupported surfaces remain rejected centrally.
- Added regression coverage for surface registration and documented the complete surface model.
