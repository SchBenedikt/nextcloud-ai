...
### Changed
- **Responsive workspace and providers**: the New chat action now uses compact native sizing, shared chat content expands on large screens, notification entries use the EVA app icon, and Assistant provider labels are standardized as `Eva · Local`, `Eva · RAG`, `Eva · Tools`, and `Eva · Agent`.
