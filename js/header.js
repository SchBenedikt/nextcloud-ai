...
(function () {
	...
	function inject() {
		...
		if (/\/apps\/ragchat($|\/)/.test(location.pathname)) return;
		var right = document.getElementById('header-right');
		if (!right || !right.firstChild) return;
		if (right.querySelector('.ragchat-header-link')) return;
		...
		style.textContent =
			...
			'margin-right:6px;' +
			'}' +
			'.ragchat-header-link:hover { background:var(--color-background-hover,#f1f2f4); }' +
			'.ragchat-header-link img { width:26px;height:26px;border-radius:6px;display:block; }';
		document.head.appendChild(style);

		var a = document.createElement('a');
		a.className = 'ragchat-header-link';
		a.href = OC.webroot + '/apps/ragchat/';
		a.title = 'AI – Chat with your files';
		a.setAttribute('aria-label', 'AI – Chat with your files');
		var img = document.createElement('img');
		img.src = OC.webroot + '/apps/ragchat/img/ragchat-icon.svg';
		img.alt = 'AI';
		a.appendChild(img);
