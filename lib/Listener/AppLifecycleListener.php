...
final class AppLifecycleListener implements IEventListener {
 ...
 public function handle(Event $event):void {
  ...
  foreach(['background_chat_queue'=>'[]','background_chat_history'=>'[]','background_chat_users'=>'[]','index_scheduler_queue'=>'[]','index_scheduler_active'=>'{}','index_job_running'=>'0','index_job_stop_requested'=>'1'] as $k=>$v)$this->config->setAppValue(AppConfig::APP,$k,$v);
  ...
  try{$this->db->executeStatement('DELETE FROM *PREFIX*eva_ai_agent_state');$this->db->executeStatement('DELETE FROM *PREFIX*eva_ai_chunks');$this->db->executeStatement('DELETE FROM *PREFIX*eva_ai_documents');}catch(\Throwable){}
