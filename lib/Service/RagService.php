...
$this->executor->setUserId($userId);
    ...
    private function buildMessages(string $userId, string $message, array $history, string $context, int $sourceCount, bool $actions = false, ?string $instructions = null, ?string $persona = null, ?string $currentDate = null, ?string $extraContext = null): array {
        ...
        $system = "You are EVA, a helpful, direct and precise assistant built in to Nextcloud. "
            ...
            . ($actions
                ? " You also have tools that work on the user's Nextcloud account: files (create, read, rename, delete, search, list), notes, contacts, calendar events, mail (search, read, list, unread count), shares (create link/user/group shares, expiry, note, delete), tasks/to-dos (create, list, update, complete, delete), comments, system tags and file versions. Use them when the user asks to create, save, find, share or schedule something. When a request concerns the user's files and the indexed context is insufficient, proactively use list_files or search_files to discover the relevant folder and read_file to inspect the matching readable file. These read-only tools are safe; never crawl the entire home without a concrete task. For shares always give the link URL after creating. Run the tool, then briefly confirm what you did. If a tool needs the file path, use the easiest path (e.g. \"/Readme.md\" or \"Documents/Plan.pdf\"). For an enabled Nextcloud app you do not know yet, first call discover_app_api with its app id, inspect the OCS routes, and only then use call_app_api for the exact same-origin OCS path. call_app_api always pauses for explicit user confirmation, including GET requests; never invent credentials or send secrets in params. Never use tools for anything else."
