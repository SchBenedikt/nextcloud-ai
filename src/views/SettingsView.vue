	...
	<div class="settings-view">
		...
		<main class="settings-body">
			...
			<section id="settings-safety" class="settings-section">
				...
				<p v-if="terminalCommandsEnabled" class="field-help">{{ $t('You can also ask EVA to run up to five allowlisted commands sequentially; each command is shown and timed separately. Confirmed interactive prompts may receive bounded stdin input and are closed with EOF.') }}</p>
	...
	setup() {
		...
		function applyConnectorExample(type) {
			connectorDraft.value = type === 'immich'
				? { id: 'immich', name: 'Immich', base_url: 'http://immich.local:2283', openapi_url: '', auth_type: 'api_key', token: '', username: '', password: '', api_key: '', api_key_header: 'x-api-key' }
				: type === 'vaultwarden'
					? { id: 'vaultwarden', name: 'Vaultwarden', base_url: 'https://vaultwarden.local', openapi_url: '', auth_type: 'bearer', token: '', username: '', password: '', api_key: '', api_key_header: 'X-API-Key' }
					: type === 'truenas'
				? { id: 'truenas', name: 'TrueNAS', base_url: 'https://truenas.local', openapi_url: '', auth_type: 'bearer', token: '', username: '', password: '', api_key: '', api_key_header: 'X-API-Key' }
				: type === 'github'
					? { id: 'github', name: 'GitHub', base_url: 'https://api.github.com', openapi_url: '', auth_type: 'bearer', token: '', username: '', password: '', api_key: '', api_key_header: 'X-API-Key' }
					: { id: 'homeassistant', name: 'Home Assistant', base_url: 'http://homeassistant.local:8123', openapi_url: '', auth_type: 'bearer', token: '', username: '', password: '', api_key: '', api_key_header: 'X-API-Key' }
		}
		...
		async function saveConnector() {
			...
			try {
				...
				connectorDraft.value = { id: '', name: '', base_url: '', openapi_url: '', auth_type: 'bearer', token: '', username: '', password: '', api_key: '', api_key_header: 'X-API-Key' }
...
.briefing-card-actions { flex-wrap:wrap; }
...
.connector-row { display:flex; align-items:center; justify-content:space-between; gap:16px; margin:12px 0; padding:16px; border:1px solid var(--color-border); border-radius:var(--border-radius-large); background:var(--color-background-hover); }
...
.connector-endpoints li small + small { color:var(--color-text-light); }
...
.auth-choice .checkbox-radio-switch { margin:0; }
.connector-discovery-status { display:grid; gap:4px; margin:12px 0 0; padding:10px 12px; border-left:3px solid var(--color-primary-element); color:var(--color-text-maxcontrast); font-size:12px; }
.connector-discovery-status span { overflow-wrap:anywhere; }
